...
const ReportFormMVP = ({ onReport, onClose }) => {
      ...
      try {
        ...
        if (addressData && !addressData.error) {
          ...
          if (filledFields.length > 0) {
            if (processedAddress.barangayNotDetected) {
              setSuccess(`Location detected! Auto-filled: ${filledFields.join(', ')}. ⚠️ Barangay not detected — please select manually.`);
            } else {
            ...
            setSuccess(`Location detected: ${detectedLocation}. ${detectedLocation.toLowerCase().includes('negros')
  ...
  const startCamera = async () => {
    try {
      ...
      // Request higher resolution + frame rate for sharp captures
      const constraints = {
        video: {
      ...
      // Apply exposure & white balance constraints to prevent overexposure
      try {
        ...
        if (capabilities.exposureMode) {
          ...
          console.log('📷 Applied continuous exposure mode');
        }
        if (capabilities.whiteBalanceMode) {
          advancedConstraints.push({ whiteBalanceMode: 'continuous' });
          console.log('📷 Applied continuous white balance mode');
        }
        if (capabilities.exposureCompensation) {
          // Reduce exposure compensation to prevent overexposure
          const minExp = capabilities.exposureCompensation.min || -2;
          advancedConstraints.push({ exposureCompensation: Math.max(minExp, -0.5) });
          console.log('📷 Reduced exposure compensation');
        }

        if (advancedConstraints.length > 0) {
          await videoTrack.applyConstraints({ advanced: advancedConstraints });
          console.log('✅ Camera constraints applied:', advancedConstraints);
        }
        ...
        console.warn('⚠️ Could not apply camera constraints (non-critical):', constraintErr);
      }

      setStream(mediaStream);
      setShowCamera(true);
      setSuccess('Stabilizing camera...');
      ...
      // Wait for camera to stabilize (auto-exposure/focus settle)
      setTimeout(() => {
  ...
  const capturePhoto = async () => {
    ...
    // CRITICAL: Use full native video resolution for maximum sharpness
    ...
    // Save at high JPEG quality (0.92) to preserve sharpness
    canvas.toBlob((blob) => {
      if (blob) {
      ...
      if (exifData && (exifData.latitude && exifData.longitude)) {
        if (exifData.latitude >= -90 && exifData.latitude <= 90 &&
          ...
          if (addressData && !addressData.error) {
            ...
            const parts = [processedAddress.city, processedAddress.province].filter(Boolean);
            if (processedAddress.barangayNotDetected) {
              setSuccess(`Photo location: ${parts.join(', ')}. ⚠️ Barangay not detected — please select manually.`);
            } else {
              setSuccess(`Photo location: ${parts.join(', ')}`);
            }
          }
