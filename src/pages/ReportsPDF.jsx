...
const ReportsPDF = () => {
  ...
  const generateMapImage = async (lat, lng) => {
    try {
      ...
      // Fallback to OpenStreetMap static image (no API key needed)
      try {
        const osmUrl = `https://staticmap.openstreetmap.de/staticmap.php?center=${lat},${lng}&zoom=15&size=400x300&markers=${lat},${lng},lightblue`;
